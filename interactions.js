...
const tablePlate=new THREE.Group();island.add(tablePlate);tablePlate.position.set(15.6,1.02,2.8);cyl(.28,.24,.055,cream,tablePlate);const tableFood=ball(.2,mat('#d9ac65'),tablePlate,0,.09,0);tableFood.scale.y=.4;tablePlate.visible=false;
const stoveGlow=cyl(.25,.25,.018,new THREE.MeshBasicMaterial({color:'#eeab51'}),island,-.2,1.375,-4.75);stoveGlow.visible=false;
const flowerGroup=new THREE.Group();island.add(flowerGroup);for(let i=0;i<4;i++){cyl(.015,.015,.42,mat('#76975f'),flowerGroup,4+i*1.2,1.1,5.5);ball(.09,mat(i%2?'#ecbb5e':'#d99998'),flowerGroup,4+i*1.2,1.34,5.5);}flowerGroup.position.x=9;flowerGroup.visible=false;
...
drawTV(0);
const at={fridge:[2.22,-3.5],board:[.8,-2.24],stove:[-.2,-3.6],sink:[-1.8,-3.65],sofa:[-5.7,4.2],tv:[-3.95,1.6],wardrobe:[-7.35,-2],lamp:[-7.8,-3.95],bed:[-6,-2.02],basin:[4.55,-3.52],toilet:[7.4,-3.52],shower:[7.65,-1.55],garden:[15.2,4.45],chair:[14.2,3.6]};
